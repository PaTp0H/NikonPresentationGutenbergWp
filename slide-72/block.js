...
const slideAttributes = {
	background: {
		...
		default: imgSrc('drive/bg-drive.png')
	},
	slideTitle: {
		type: 'string',
	...
	items: {
		...
		default: [
			{
				title: 'THE REFLECTION OF LIGHT ON THE LENS<br>IN DARK CONDITIONS CREATE DISTURBING HALOS.',
				image: imgSrc('drive/dv-technology-01.jpg'),
				navTitle: 'CONVENTIONAL LENS',
				useImage: false,
			},
			{
				title: 'THE LIGHT REFLECTION ON LENSES<br>AT NIGHT IS MINIMIZED. <br><span class="np-style-font-weight-extra-bold"><span class="np-style-letter-spacig-large">PERCEPTION OF GLARE IS REDUCED.</span></span>',
				image: imgSrc('drive/dv-technology-02.jpg'),
				useImage: true,
				navImage: imgSrc('drive/icon-drive.svg'),
			},
		],
		query: {
			title: {
...
registerBlockType('presentation/slide72', {
	title: __('Slide: 72', 'np'),
	icon: icon,
	...
	(({activeTab, setState, className, attributes, setAttributes}) => {
		...
		const item = i => {
			...
			return i === activeTab &&
				<div className="wd-sharper__content active js-tab-content" id={`wd-shaper-item-${i}`}>
        ...
        <span className="gold">
          ...
          />
			...
			<div className="screen_body_wrapper start bottom bottom--mod no_relative">
				<div className="wd-sharper wd-sharper--drive opacity_animate">
	...
	save: ({attributes}) => {
		...
		const item = i => {
			...
			if (attributes.items[i].useImage)
				metaAtts['data-use-image'] = 'true';
			return <div className={(i === activeTab ? 'active ' : ' ') +"wd-sharper__content js-tab-content"} id={`wd-shaper-item-${i}`}>
				<div className="meta" {...metaAtts}>
					<div className="nav-title">{attributes.items[i].navTitle}</div>
					<img src={attributes.items[i].navImage} alt="" className="nav-image"/>
				</div>
      ...
      <div className="brand_wrapper">
        <span className="gold">
          <RichText.Content value={attributes.slideTitle}/>        </span>
      </div>
