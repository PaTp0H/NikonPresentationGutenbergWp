...
const slideAttributes = {
	...
	imageUrl1: {
		...
		default: imgSrc("power/power-img.jpg")
	},
	imageUrl2: {
		attribute: 'src',
	...
	screenName: {
		...
		source: 'html',
		default: 'Vision<br>As It Should Be'
	},
	flipped: {
		type: 'boolean',
		default: false
...
registerBlockType('presentation/slide92', {
	title: __('Slide: #92', 'np'),
	icon: icon,
	...
	edit({className, attributes, setAttributes}) {
		...
		const rightSide = <div className="title-screen__inner right-slide">
			<div className="title-screen__info">
				...
				<div className="title-screen__name">
					<RichText
						...
						placeholder={slideAttributes.screenName.default.replace(/(<([^>]+)>)/ig,"")}
					/>
				</div>
			</div>
		</div>;
		...
		return <div className={className + " block-presentation-slide all-empty"}>
			...
			<InspectorControls>
				...
				/>
				<br/>
				<h3>{__('Flip layout', 'np')}</h3>
				<ToggleControl
					label={__("Flip layout?", 'np')}
					help={attributes.flipped ? __('Left', 'np') : __('Right', 'np')}
					checked={attributes.flipped}
					onChange={() => setAttributes({flipped: !attributes.flipped})}
				/>
